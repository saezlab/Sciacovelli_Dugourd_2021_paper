...
sub_network <- sub_network_nocofact$reaction_network
sub_network$interaction <- 1
sub_network <- cosmosR:::keep_controllable_neighbours(network = sub_network, n_steps = 12, input_nodes = c("cpd:C00123_c","cpd:C00407_c"))

node_att <- sub_network_nocofact$attributes
node_att <- node_att[node_att$V1 %in% sub_network$source | node_att$V1 %in% sub_network$target,]
sub_network <- sub_network[,-3]

sub_network_nocofact$reaction_network <- sub_network
sub_network_nocofact$attributes <- node_att
...
##Visualise results for single enzmes
plots <- plotMetaboliteContribution(enzyme = 'BCAT1_nitro', stat_df = translated_results$t_table, 
                                    metabolite_sets = translated_regulons_df, 
                                    contrast_index = 4, stat_name = 'Abundance Down <==> Up (t-value)', 
                                    scaling_factor = 150, nLabels =  30)
...
##Visualise results for single enzmes
plots <- plotMetaboliteContribution(enzyme = 'GPT2>1060_nitro', stat_df = translated_results$t_table, 
                                    metabolite_sets = translated_regulons_df, 
                                    contrast_index = 4, stat_name = 'Abundance Down <==> Up (t-value)', 
                                    scaling_factor = 150, nLabels =  30)
...
##Visualise results for single enzmes
plots <- plotMetaboliteContribution(enzyme = 'GLS', stat_df = translated_results$t_table, 
                                    metabolite_sets = translated_regulons_df, 
                                    contrast_index = 4, stat_name = 'Abundance Down <==> Up (t-value)', 
                                    scaling_factor = 150, nLabels =  30)
